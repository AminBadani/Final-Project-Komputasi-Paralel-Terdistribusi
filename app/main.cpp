...
int main(int argc, char** argv) {
    ...
    int world_rank, world_size, world_len;
    double start_global, end_global, duration_global;
    ...
    // Broadcast the config from Master (0) to ALL workers
    ...
    // Process local sum on each workers
    ...
    if (world_rank != 0) {
        double local_result[2] = {local_sum, duration_local};
        long long local_range[2] = {start, end};
        MPI_Send(local_result, 2, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD);
    ...
    if (world_rank == 0) {
        // Print from Master
        std::cout << "\nContainer Rank " << 0
                << " computed range [" << start << " to " << end << "]" 
                << " using " << current_config.active_cores << " cores" 
                << " \n   -- Local sum: " << std::fixed << local_sum  << "."
                << " \n   -- Local duration: " << duration_local << " seconds.\n";

        // Send each workers result to Master
        double worker_result[2];
        long long worker_range[2];
        for (int i = 1; i < world_size; i++) {
            MPI_Recv(worker_result, 2, MPI_DOUBLE, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(worker_range, 2, MPI_LONG_LONG, i, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            auto [worker_sum, worker_duration] = worker_result;
            auto [worker_start, worker_end] = worker_range;

            std::cout << "\nContainer Rank " << i 
                << " computed range [" << worker_start << " to " << worker_end << "]" 
                << " using " << current_config.active_cores << " cores" 
                << " \n   -- Local sum: " << std::fixed << worker_sum  << "."
                << " \n   -- Local duration: " << worker_duration << " seconds.\n";
        }
        ...
        std::cout << "=== TOTAL TIME: " << duration_global << " seconds.\n";
    }
